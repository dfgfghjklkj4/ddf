...
public class PlayerController : MonoBehaviour
{
    ...
    private void Teleport(Vector3 position)
    {
        ...
        var effect = Instantiate(teleportEffect, this.camera.transform.position, Quaternion.identity);
    }
  ...
  public  BlockType currentType;
    ...
    private void PlayStepAudio()
    {
       ...
       //  Vector3Int positionBeneathFeet = _currPosition + Vector3Int.down * 2;
          //  Vector3Int positionBeneathFeet =new Vector3Int(feet.position.x,feet.position.y,feet.position.z);
            ...
            if (!BlockType.IsAirBlock(currentType))
            {
                ...
                if (stepClips != null && stepClips.Length > 0)
                {
                    ...
                    stepAudioSource.clip = clip;
                     stepAudioSource.Play();
                  //  print(currentType.blockName+"  "+ clip.name);
    ...
    private void PlaceBlock(BlockType block)
    {
        ...
        if (RaycastToBlock(10.0f, out RaycastHit hit, true, out blockPos))
        {
            ...
            /*
               if (chunk.Blocks[0,0,0]==null &&chunk. computedTerrainDate)
            {
            ...
            foreach (var pos in positions)
            {
                ...
                if (isAirBlock)
                {
                    ...
                    if (!BlockType.IsAirBlock(blockBeneath) && blockBeneath.blockName == BlockNameEnum.Grass && Random.value > 0.5f)
                    {
                        ...
                        if (isGrassBlock)
                        {
                            blockTypeChoices = BlockType.PlantBlocks;
                        }
                        else
                        {
                ...
                var effect = Instantiate(growthEffect, null) ;
